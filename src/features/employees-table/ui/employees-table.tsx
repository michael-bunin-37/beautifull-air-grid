...
function EmployeesTable({ className }: EmployeesTableProps) {
	...
	const t = useTranslations("employees-table.status")
	...
	const { table, virtualizer, scrollContainerRef } = useEmployeesTable({
		data: deferredData,
		params,
	})
					...
					<TableBody>
						...
						{!isPending && !isEmpty && (
							<>
								{paddingTop > 0 && (
									<tr>
										<td colSpan={columnsCount} style={{ height: paddingTop }} />
									</tr>
								)}
								{virtualRows.map((virtualRow) => {
									const row = rows[virtualRow.index]
									return (
										<EmployeesTableVirtualRow key={row.id} row={row} virtualRowIndex={virtualRow.index} />
									)
								})}
								{paddingBottom > 0 && (
									<tr>
										<td colSpan={columnsCount} style={{ height: paddingBottom }} />
									</tr>
								)}
							</>
						)}
